...
export async function getContactoPorEmail(email) {
  ...
  const encontrado = (data.data || [])
    .find((c) => String(c.attributes?.email || '').toLowerCase().trim() === String(email).toLowerCase().trim());
  if (!encontrado) return null;
  let contacto = encontrado.attributes || {};
  // DNI/NIE: es un campo personalizado ("DNI/NIE") que viaja como atributo con
  // su handle propio — se busca por FORMATO en todos los atributos del
  // contacto, y si el listado no trae los personalizados, se pide la ficha
  // completa del contacto.
  const esDni = (v) => typeof v === 'string' && /^[XYZ]?\d{7,8}[A-Z]$/i.test(v.replace(/[\s.-]/g, ''));
  const buscarDni = (attrs) => Object.values(attrs || {}).find(esDni) || null;
  let dni = buscarDni(contacto);
  if (!dni) {
    const ficha = await axios.get(`${KAJABI_API}/v1/contacts/${encontrado.id}`, { headers: cabeceras }).catch(() => null);
    if (ficha?.data?.data?.attributes) {
      contacto = { ...ficha.data.data.attributes, ...contacto };
      dni = buscarDni(ficha.data.data.attributes);
    }
  }
  const direccion = [
  ...
  return {
    ...
    movil: contacto.mobile_phone_number || null,
  };
}
