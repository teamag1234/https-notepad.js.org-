...
// Numero del equipo de atencion al alumno (tutores)
const SOPORTE_TELEFONO = '+34 601 99 35 80';
const SOPORTE_WA = 'https://wa.me/34601993580';
const soporteHtml = `
        <p style="background: #f2f4f7; padding: 12px 16px; border-radius: 6px; color: #444;">
          📞 <strong>¿Dudas o problemas?</strong> Nuestro equipo de atención al alumno te ayuda por WhatsApp:
          <a href="${SOPORTE_WA}" style="color: #128C7E; font-weight: bold; text-decoration: none;">${SOPORTE_TELEFONO}</a>
        </p>`;
...
export async function sendWhatsAppOnboardingEmail(studentEmail, studentName, courseName, acceptUrl, extraNote = '') {
  try {
    console.log(`Sending WhatsApp onboarding email to ${studentEmail}`);

    const mailOptions = {
      from: process.env.EMAIL_FROM || process.env.EMAIL_USER,
      to: studentEmail,
      subject: `🎓 Tu acceso a ${courseName} — completa tu alta`,
      html: `
        <h2>¡Bienvenido/a ${studentName}!</h2>
        <p>Ya estás inscrito/a en <strong>${courseName}</strong>. El curso funciona a través de una comunidad privada de WhatsApp, donde recibirás los contenidos, avisos y soporte.</p>
        ${extraNote ? `<p style="background: #FFF6DD; border-left: 4px solid #F5A623; padding: 12px 16px; border-radius: 4px;">${extraNote}</p>` : ''}
        <p>Para completar tu alta solo falta un paso: leer y aceptar las condiciones del curso. Pulsa este botón (es tu enlace personal, no lo compartas):</p>
        <p style="margin: 24px 0;">
          <a href="${acceptUrl}" style="background: #25D366; color: #fff; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: bold;">
            👉 Completar mi alta
          </a>
        </p>
        <p>Al aceptar las condiciones recibirás automáticamente tu acceso a la comunidad de WhatsApp del curso.</p>
        ${soporteHtml}
        <p>Un saludo,<br>El equipo de AG Academy</p>
      `,
    };
    ...
    const info = await transporter.sendMail(mailOptions);
    console.log(`WhatsApp onboarding email sent to ${studentEmail}:`, info.response);
    return true;
  } catch (error) {
    console.error(`Error sending WhatsApp onboarding email to ${studentEmail}:`, error.message);
    return false;
  }
}

export async function sendWhatsAppConsentConfirmationEmail(studentEmail, studentName, courseName, conditionsUrl, conditionsVersion, acceptedAt, whatsappLink, videoUrl = null) {
  try {
    console.log(`Sending WhatsApp consent confirmation email to ${studentEmail}`);
    ...
    const mailOptions = {
      ...
      html: `
        ...
        <ul>
          <li><strong>Documento aceptado:</strong> <a href="${conditionsUrl}">Condiciones del curso</a> (versión ${conditionsVersion})</li>
          <li><strong>Fecha y hora de aceptación:</strong> ${acceptedDate} (hora peninsular española)</li>
          <li><strong>Medio:</strong> confirmación expresa en la página de aceptación del curso</li>
          <li><strong>Email asociado:</strong> ${studentEmail}</li>
        </ul>
        <p>Y este es tu acceso a la comunidad del curso:</p>
        <p style="margin: 24px 0;">
          <a href="${whatsappLink}" style="background: #25D366; color: #fff; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: bold;">
            💬 Entrar a la comunidad de WhatsApp
          </a>
        </p>
        ${videoUrl ? `
        <p>🎯 <strong>Tu vídeo de guía y planificación:</strong> míralo antes de empezar para aprovechar el curso al máximo 💪</p>
        <p style="margin: 16px 0;">
          <a href="${videoUrl}" style="background: #075E54; color: #fff; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: bold;">🎬 Ver mi vídeo de guía</a>
        </p>` : ''}
        <p style="background: #FFF6DD; border-left: 4px solid #F5A623; padding: 12px 16px; border-radius: 4px;">
          🔒 <strong>¿Tienes módulos bloqueados?</strong> Es normal: tienes una <strong>prueba de 15 días</strong>.
          Si quieres desbloquear todo ya mismo y no perderte nada, escríbenos por WhatsApp con el mensaje
          <strong>"Quiero desbloquear"</strong> ¡y nosotros nos encargamos del resto! 😉<br><br>
          <a href="${SOPORTE_WA}?text=Quiero%20desbloquear" style="background: #F5A623; color: #fff; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-weight: bold;">🔓 Quiero desbloquear</a>
        </p>
        <p>Te recomendamos guardar este email. Puedes consultar las condiciones en cualquier momento en el enlace anterior.</p>
        <p>Si no has sido tú quien ha realizado esta aceptación, responde a este email inmediatamente.</p>
        ${soporteHtml}
    ...
    const info = await transporter.sendMail(mailOptions);
    console.log(`WhatsApp consent confirmation email sent to ${studentEmail}:`, info.response);
    return true;
  } catch (error) {
    console.error(`Error sending WhatsApp consent confirmation email to ${studentEmail}:`, error.message);
    return false;
  }
}
