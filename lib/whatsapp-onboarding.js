...
export async function startWhatsAppOnboarding({ name, email, courseName }) {
  ...
  if (existing) {
    if (existing.fields['Estado'] === ESTADO_WA.PENDIENTE) {
      ...
      await sendWhatsAppOnboardingEmail(email, name, canonicalName, getAcceptUrl(existing.fields['Token']), aviso);
    } else {
  ...
  await sendWhatsAppOnboardingEmail(email, name, canonicalName, getAcceptUrl(token), aviso);
  return record;
}
...
export async function getAcceptanceInfo(token) {
  ...
  return {
    ...
    aceptado: record.fields['Estado'] === ESTADO_WA.ACEPTADO,
    // El enlace de WhatsApp solo se expone cuando ya ha aceptado
    whatsappLink: record.fields['Estado'] === ESTADO_WA.ACEPTADO ? course.fields['Enlace WhatsApp'] : null,
  };
}
...
export async function acceptConditions(token, ip, userAgent) {
  ...
  if (fields['Estado'] === ESTADO_WA.ACEPTADO) {
    return { whatsappLink, alreadyAccepted: true };
  }
  ...
  return { whatsappLink, alreadyAccepted: false };
}
...
export async function checkPendingWhatsAppOnboardings() {
  ...
  for (const record of records) {
    ...
    const aviso = course?.fields['Aviso Email'] || '';
    const ok = await sendWhatsAppOnboardingEmail(fields['Email'], fields['Nombre'], fields['Curso'], getAcceptUrl(fields['Token']), aviso);
    if (ok) {
      await updateRecord(ONBOARDING_TABLE, record.id, {
