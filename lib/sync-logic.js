...
export function cuotasDeOferta(t) {
  const desc = String(t.ofertaPrecioDesc || '');
  const m = desc.match(/(\d+)\s*(?:[a-záéíóú]+\s+)?(pagos?|payments?|cuotas?|plazos?)/i);
...
export async function esPrimerPago(t, fecha) {
  const filas = await q(
  ...
  for (const t of transacciones) {
    try {
      ...
      if (estado === 'failed') {
        ...
        const anotFallo = await aplicarFallidoKajabi({ email: t.email, nombre: t.nombre, importe: t.importe, fecha }).catch((e) => {
