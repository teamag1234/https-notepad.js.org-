...
export async function sincronizarBanco({ desde: desdeForzado } = {}) {
  await ensureSchema();
  ...
  for (const cuenta of cuentas) {
    // Primera vez: 90 días atrás; después: desde la última sync con 5 días de
    // margen. Con desdeForzado (p. ej. 2026-01-01) se pide el histórico entero
    // que el banco permita servir.
    const desde = desdeForzado
      ? new Date(desdeForzado)
      : cuenta.ultima_sync
