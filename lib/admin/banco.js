...
// Reconstruye el PEM aunque al pegarlo en la variable de entorno se hayan
// perdido los saltos de línea, venga en una sola línea, con comillas o con \n escapados.
function normalizarPem(bruto) {
  let s = String(bruto || '').trim().replace(/\\n/g, '\n').replace(/^["']+|["']+$/g, '').trim();
...
export function diagnosticoClave() {
  ...
  try {
    crypto.createSign('RSA-SHA256').update('diagnostico').sign(pem, 'base64url');
...
function jwtEnableBanking() {
  ...
  const pem = normalizarPem(process.env.ENABLE_BANKING_PRIVATE_KEY);
  const firma = crypto.createSign('RSA-SHA256').update(`${header}.${payload}`).sign(pem, 'base64url');
