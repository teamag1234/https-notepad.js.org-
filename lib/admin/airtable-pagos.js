...
// ── Alumnos ──────────────────────────────────────────────────────────────────
export async function buscarAlumno(email, nombre) {
...
async function crearAlumno({ nombre, email, curso, fechaInscripcion }) {
  const fields = { 'Name': nombre || email, 'Email': email };
  if (curso) fields['TIPO DE CURSO'] = curso;
  if (fechaInscripcion) {
    // El campo real lleva espacio final ("FECHA INSCRIPCIÓN ")
    const campo = (await campoReal('FECHA INSCRIPCIÓN')) || 'FECHA INSCRIPCIÓN ';
    fields[campo] = fechaInscripcion;
  }
...
export async function aplicarPagoKajabi(pago) {
  ...
  if (!alumno) {
    alumno = await crearAlumno({ nombre: pago.nombre, email: pago.email, curso: cursoDesdeOferta(pago.oferta), fechaInscripcion: pago.fecha });
  ...
  for (const pago of pagos) {
    try {
      ...
      if (!a) {
        ...
        if (!record) {
          record = await crearAlumno({ nombre: pago.nombre, email: pago.email, curso: cursoDesdeOferta(pago.oferta), fechaInscripcion: pago.fecha });
