...
export async function listarFacturas() {
  ...
  return q(`
    SELECT id, numero, to_char(fecha, 'DD/MM/YYYY') AS fecha, alumno, dni, direccion, email,
           concepto, cantidad, importe::float AS importe, estado, pagada,
...
export async function crearFactura({ fecha, alumno, dni, direccion, email, concepto, cantidad, importe, referencia, pagada }) {
  await ensureSchema();
  if (!alumno || !concepto || !importe) throw new Error('Faltan alumno, concepto o importe');
  const numero = await siguienteNumero();
  const filas = await q(`
    INSERT INTO facturas (numero, fecha, alumno, dni, direccion, email, concepto, cantidad, importe, referencia, pagada)
  ...
  `, [numero, fecha || new Date().toISOString().slice(0, 10), alumno.trim(), dni || null, direccion || null,
      email || null, concepto, Number(cantidad) || 1, importe, referencia || null, pagada !== false]);
  return filas[0];
}

export async function actualizarFactura(id, campos) {
  const permitidos = ['fecha', 'alumno', 'dni', 'direccion', 'email', 'concepto', 'cantidad', 'importe', 'pagada'];
...
export async function pdfDesdeDatos(f) {
  ...
  if (f.pagada) {
    // Sello PAGADA (verde) alineado a la derecha
    doc.save();
    doc.roundedRect(469, 110, 74, 18, 5).lineWidth(1.5).stroke('#059669');
    doc.font('Helvetica-Bold').fontSize(11).fillColor('#059669').text('PAGADA', 469, 114, { width: 74, align: 'center' });
    doc.restore();
  }
  ...
  // Forma de pago: si ya está pagada no se muestran datos de pago
  const yPago = yTotal + 64;
  if (f.pagada) {
    doc.roundedRect(50, yPago - 4, 300, 30, 6).fill('#ecfdf5');
    doc.font('Helvetica-Bold').fontSize(10).fillColor('#059669')
      .text('Factura PAGADA — no es necesario realizar ningún pago.', 62, yPago + 5, { width: 280 });
  } else {
  ...
  await t.sendMail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 540px;">
        ...
        ${factura.pagada ? '<p style="background:#ecfdf5;border-radius:8px;padding:10px 14px;color:#059669;"><strong>✔ Esta factura ya está pagada</strong> — no tienes que hacer nada, es solo tu justificante.</p>' : ''}
