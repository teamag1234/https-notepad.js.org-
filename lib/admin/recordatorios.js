...
// Cadencia de seguimiento: los 3 primeros recordatorios automáticos van cada
// 2 días; si el alumno sigue sin pagar, se pasa a uno diario.
const ENVIOS_CADA_DOS_DIAS = 4; // 1 manual + 3 seguimientos
function horasDeIntervalo(enviadosHastaAhora) {
  return enviadosHastaAhora >= ENVIOS_CADA_DOS_DIAS ? 24 : 48;
}
...
export function plantillaRecordatorio(moroso) {
  // En el email se muestra solo el nombre del mes, sin el año de la columna
  // interna de Airtable ("JUNIO 25" → "JUNIO", "NOVIEMBRE 2025" → "NOVIEMBRE")
  const detalle = (moroso.detalle || '')
    .replace(/^Pendiente:\s*/i, '')
    .replace(/\b(ENERO|FEBRERO|MARZO|ABRIL|MAYO|JUNIO|JULIO|AGOSTO|SEPTIEMBRE|OCTUBRE|NOVIEMBRE|DICIEMBRE)\s+(20)?\d{2}\b/gi, '$1');
...
async function yaAvisadoHacePoco(morosoId) {
  ...
  const filas = await q(
    `SELECT MAX(enviado_el) AS ultimo, COUNT(*)::int AS enviados
     FROM recordatorios_log WHERE alumno_airtable_id = $1 AND modo = 'REAL'`,
    [morosoId],
  );
  if (!filas.length || !filas[0].ultimo) return false;
  const horasDesdeUltimo = (Date.now() - new Date(filas[0].ultimo).getTime()) / 3600000;
  // Margen de 3 h para que el cron diario no se salte un ciclo por minutos
  return horasDesdeUltimo < horasDeIntervalo(filas[0].enviados) - 3;
}
...
// ── Pausas individuales (p. ej. alguien paga en efectivo o hay un acuerdo) ──
export async function alumnosPausados() {
  if (!dbConfigurada()) return new Set();
  await ensureSchema();
  const filas = await q(`SELECT alumno_airtable_id FROM recordatorios_pausas`);
  return new Set(filas.map((f) => f.alumno_airtable_id));
}

export async function pausarAlumno(id, nombre) {
  await ensureSchema();
  await q(
    `INSERT INTO recordatorios_pausas (alumno_airtable_id, nombre) VALUES ($1, $2)
     ON CONFLICT (alumno_airtable_id) DO NOTHING`,
    [id, nombre || null],
  );
}

export async function reanudarAlumno(id) {
  await ensureSchema();
  await q(`DELETE FROM recordatorios_pausas WHERE alumno_airtable_id = $1`, [id]);
}
...
export async function enviarUno(morosoId) {
  await ensureSchema();
  const pausados = await alumnosPausados();
  if (pausados.has(morosoId)) {
    throw new Error('Este alumno está en pausa (pago en efectivo/acuerdo). Reanúdalo primero si quieres enviarle.');
  }
  const { conEmail } = await getMorososParaAvisar();
...
// Seguimientos automáticos (cron diario): SOLO a los alumnos a los que ya se
// les envió el primer recordatorio a mano y que sigan siendo morosos. Cadencia:
// cada 2 días los primeros avisos y diario si sigue sin pagar. Cuando el pago
// se concilia, salen de morosos y el seguimiento se corta solo.
export async function enviarSeguimientos() {
  ...
  for (const moroso of conEmail) {
    if (pausados.has(moroso.id)) continue; // en pausa (efectivo/acuerdo): no enviar
