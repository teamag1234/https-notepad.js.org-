...
export async function getResumenFinanzas() {
  const [porMes, categoriasMes, ultimos, kpisMes] = await Promise.all([
    q(`
      SELECT to_char(fecha, 'YYYY-MM') AS mes,
             SUM(CASE WHEN tipo = 'INGRESO' THEN importe ELSE 0 END)::float AS ingresos,
    ...
    q(`
      SELECT
        COALESCE(SUM(CASE WHEN tipo = 'INGRESO' THEN importe END), 0)::float AS ingresos,
        COALESCE(SUM(CASE WHEN tipo = 'GASTO' THEN importe END), 0)::float AS gastos,
        COUNT(*) FILTER (WHERE tipo = 'INGRESO') AS num_cobros
      FROM movimientos
      WHERE to_char(fecha, 'YYYY-MM') = $1
    `, [mesActual()]),
  ]);
  ...
  return {
    ...
    mesActual: {
      ...
      gastos: kpi.gastos,
      beneficio: Math.round((kpi.ingresos - kpi.gastos) * 100) / 100,
