...
// Email para los avisos de documentos/nóminas: por confidencialidad se usa una
// cuenta aparte (RRHH_EMAIL_USER / RRHH_EMAIL_PASSWORD) si está configurada,
// porque al buzón del equipo (EMAIL_USER) tiene acceso todo el mundo. Sin esas
// variables se usa el email general, como antes.
function emailRrhh() {
  if (process.env.RRHH_EMAIL_USER && process.env.RRHH_EMAIL_PASSWORD) {
    return {
      user: process.env.RRHH_EMAIL_USER,
      pass: process.env.RRHH_EMAIL_PASSWORD,
      from: process.env.RRHH_EMAIL_FROM || process.env.RRHH_EMAIL_USER,
      confidencial: true,
    };
  }
  return {
...
export function emailRrhhInfo() {
  const cuenta = emailRrhh();
  return { remitente: cuenta.user || null, confidencial: cuenta.confidencial };
}
...
export async function listarTrabajadores() {
  ...
  const trabajadores = await q(`
    SELECT t.id, t.nombre, t.email, t.email_rrhh, t.puesto, to_char(t.fecha_alta, 'YYYY-MM-DD') AS fecha_alta,
...
export async function actualizarTrabajador(id, campos) {
  const permitidos = ['nombre', 'email', 'email_rrhh', 'puesto', 'fecha_alta', 'dias_vacaciones', 'pin', 'activo', 'notas'];
  const sets = [];
  const valores = [];
...
export async function crearDocumento({ trabajador_id, tipo, titulo, mes, url, origen }) {
  ...
  let avisado = false;
  try {
    // Si el trabajador tiene email de RRHH, las nóminas van ahí (el email
    // principal puede ser una bandeja a la que accede más gente)
    const t = await q(`SELECT nombre, COALESCE(NULLIF(TRIM(email_rrhh), ''), email) AS email FROM trabajadores WHERE id = $1`, [trabajador_id]);
    const trabajador = t[0];
    const cuenta = emailRrhh();
    if (trabajador?.email && cuenta.user && origen) {
