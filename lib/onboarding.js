...
// Kajabi puede mandar el titulo de la oferta con comillas, espacios extra o
// distinta capitalizacion ("Level Up with Jesu" vs Level Up with Jesu):
// comparamos siempre la version normalizada.
...
export async function startOnboarding({ name, email, courseName }) {
  // payment.succeeded llega con TODOS los pagos de Kajabi: solo hacemos
  // onboarding si la oferta esta dada de alta en la tabla de cursos.
  const course = await findCourse(courseName);
  if (!course) {
    console.log(`Oferta sin curso de Telegram configurado, ignorada: "${courseName}"`);
    return { ignored: true };
  }
  ...
  if (existing) {
    ...
    if (existing.fields['Estado'] === ESTADO.PENDIENTE) {
      ...
      await sendTelegramOnboardingEmail(email, name, displayName, deepLink, course.fields['Aviso Email'] || '');
    } else {
...
export async function checkPendingOnboardings() {
  ...
  for (const record of records) {
    ...
    const course = await findCourse(fields['Curso']);
    const ok = await sendTelegramOnboardingEmail(fields['Email'], fields['Nombre'], course?.fields['Nombre Público'] || fields['Curso'], deepLink, course?.fields['Aviso Email'] || '');
    if (ok) {
      await updateRecord(ONBOARDING_TABLE, record.id, {
